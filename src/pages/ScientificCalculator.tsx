...
const ScientificCalculator = ():ReactElement => {
  ...
  const savePreviousResult = (e:OperationEvent):void => {
    e.preventDefault();
    setPrevResult && setPrevResult((prevRes: number[])=> prevRes.length < 5 ? prevRes = [...prevRes, result] : prevRes = [...prevRes.slice(1), result])
  };
  ...
  let val = (v:HTMLInputElement) => {
    let res;
   if(v.value.trim() !== '') {res = math.evaluate(v.value.trim());}
  ...
  const calcOperations = [
    ...
    function pow(e:OperationEvent):void {
      e.preventDefault();
      if (inputRef.current && inputRef.current.value !== null && !inputReg.test(inputRef.current.value.trim())) { throw typeError }
    ...
    function factorial(e:OperationEvent):void {
      e.preventDefault();
      if (inputRef.current && inputRef.current.value !== null && !inputReg.test(inputRef.current.value.trim())) { throw typeError }
      setResult((result) => result = math.factorial(inputRef.current && val(inputRef.current)));
    },
    ...
    function calculateCotangens(e:OperationEvent):void {
      e.preventDefault();
      if (inputRef.current && inputRef.current.value !== null && !inputReg.test(inputRef.current.value.trim())) { throw typeError }
      setResult((result) => result = math.cot(radians()));
    },
    ...
    function calculateArchCot(e:OperationEvent):void {
      e.preventDefault();
      if (inputRef.current && inputRef.current.value !== null && !inputReg.test(inputRef.current.value.trim())) { throw typeError }
      setResult(result => result = math.acot(radians()));
    },
    ...
    function calculateHyperbolicArchCot(e:OperationEvent):void {
      e.preventDefault();
      if (inputRef.current && inputRef.current.value !== null && !inputReg.test(inputRef.current.value.trim())) { throw typeError }
      setResult(result => result = math.acoth(radians()));
    },
   ...
   <div className="flex flex-col justify-center items-center">
    {message && 
       ...
       <div className="text-yellow-300 text-[0.7rem] text-center">
            ...
            autoFocus
          /></div>
          <Button operationHandler={deleting} savePreviousResult={()=>null}/>
          </div>
